...
export default function App(): JSX.Element {
  const [range, setRange] = useState<Range>("1 Month");
  const [base, setBase] = useState<Currency>("USD");
  const { data, loading, currenciesToCompare } = useCurrencyData(range, base);
  ...
  if (data.length >= 2) {
    ...
    const last = data[data.length - 1];
    currenciesToCompare.forEach((curr) => {
      if (typeof first[curr] === "number" && typeof last[curr] === "number") {
        percentageChanges[curr] = ((last[curr] as number) - (first[curr] as number)) / (first[curr] as number) * 100;
      }
    });
  }
  ...
  return (
    <div className="p-6 space-y-8">
      <h1 className="text-2xl font-bold mb-4">Currency Comparison (Base: {base})</h1>

      <CurrencyControls range={range} setRange={setRange} base={base} setBase={setBase} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {currenciesToCompare.map((curr, index) => (
              <ChartCard
                ...
                hideXAxis={range === "1 Day"}
              />
            ))}
          </div>

          <PercentageChangeTable changes={percentageChanges} range={range} />
        </>
      )}
    </div>
  );
